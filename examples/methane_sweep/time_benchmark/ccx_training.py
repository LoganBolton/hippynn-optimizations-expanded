...
def make_model(network_params, tensor_order, tensor_factors=4):
    """
...
def make_loss(henergy, force,force_training,force_lambda,energy_lambda):
    """
    ...
    losses = {
        ...
        "AtomHier": Mean.of_node(henergy.hierarchicality),
        "BatchHier": henergy.batch_hier.pred
    }


    losses["EnergyTotal"] = energy_lambda* (losses['T-RMSE'] + losses["T-MAE"] )
    ...
    if force_training:
       losses.update(force_losses)
       losses["ForceTotal"] = force_lambda*(losses["F-RMSE"] + losses["F-MAE"])
       losses["ErrorTotal"] = losses["EnergyTotal"] + losses["ForceTotal"]
    else:
       losses["ErrorTotal"] = losses['EnergyTotal']

    losses["LossTotal"] = losses["ErrorTotal"] + 30*losses["BatchHier"]
    return losses
...
def load_db(db_info, en_name, force_name, seed, anidata_location, n_workers):
    ...
    this_en = database.arr_dict[en_name]
    
    ccsd_found = ~np.isnan(ccsd_en)
    this_found = ~np.isnan(this_en)
    both_found = ccsd_found & this_found
    print("Found ccsd:",ccsd_found.sum())
    print(f"Found {en_name}:",this_found.sum())
...
def main(args):
    ...
    print("custom kernels:",hippynn.custom_kernels.CUSTOM_KERNELS_ACTIVE)
    netname = f"{args.tag}_GPU{args.gpu}_SEED{args.seed}"
    network_parameters = {
    ...
    with hippynn.tools.active_directory(netname):
        with hippynn.tools.log_terminal("training_log.txt", 'wt'):
            ...
            validation_losses = make_loss(henergy, force,force_training=args.force_training,force_lambda=args.force_lambda,energy_lambda=args.energy_lambda)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed", type=int, default=0, help='random seed for init and split, use negative to generate')

    parser.add_argument("--n_interactions", type=int, default=2)
    parser.add_argument("--n_atom_layers", type=int, default=5)
    parser.add_argument("--n_features", type=int, default=256)#128
    ...
    parser.add_argument("--tensor_factors", type=int, default=4)

    parser.add_argument("--anidata_location", type=str, default='../../release/datasets/ani1x_release/ani1x-release.h5')
    parser.add_argument("--qm_method", type=str, default='wb97x')
    parser.add_argument("--basis_set", type=str, default='dz')

    parser.add_argument("--force_training", action='store_true', default=False)#False)
    parser.add_argument("--force_lambda", type=float,default=1)
    ...
    parser.add_argument("--patience",type=int, default=5)#25
    parser.add_argument("--max_epochs",type=int, default=750)
    parser.add_argument("--stopping_key",type=str, default="ErrorTotal")
    ...
    if args.seed <= 0:
        ...
        now = int(time.time())
        pid = os.getpid()
        job_id = hash(hash(now)+hash(pid)) 
        seed = hash(args.tag)+args.gpu+job_id
        seed = hash(str(hash))
        seed = seed%1_000_000
        args.seed = seed
