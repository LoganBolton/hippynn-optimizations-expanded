...
def make_model(network_params, tensor_model, tensor_order, tensor_factors, group_norm, atomization_consistent):
    """
    ...
    if tensor_order == 0 or tensor_model == "NONE":
        ...
        if tensor_model == "TS":
            ...
            network_params["group_norm"] = group_norm
...
def load_db(db_info, en_name, force_name, seed, anidata_location, n_workers, use_ccx_subset):
    ...
    if en_name != CCX_EN_NAME:
        database.targets.remove(CCX_EN_NAME)
...
def main(args):
    ...
    with hippynn.tools.active_directory(netname):
        with hippynn.tools.log_terminal("training_log.txt", "wt"):
            ...
            print("got db targets",database.targets)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tag", type=str, default="TEST_GNCOMPARE_ccx_", help="name for run")
    ...
    parser.add_argument("--random_seed", type=bool, default=False, help="set random seed and add to tag")
    ...
    parser.add_argument("--group_norm", action=BooleanOptionalAction, default=True, help="Use group norm (HIP-HOP-NN only).")
    ...
    parser.add_argument("--stopping_key", type=str, default="LossTotal")
    ...
    parser.add_argument("--progress", action=BooleanOptionalAction, default=False, help="Whether to use progress bars.")
    parser.add_argument("--n_workers", type=int, default=0, help="workers for pytorch dataloaders")
    args = parser.parse_args()
    args.tag = f"{args.tag}_gn{args.group_norm}_{args.tensor_model}_l{args.tensor_order}_n{args.tensor_factors}"
    if args.random_seed:
        import time, os
        now = int(time.time())
        pid = os.getpid()
        ...
        seed = seed%1_000_000
        args.seed = seed
        args.tag = f"{args.tag}_seed{args.seed}"
