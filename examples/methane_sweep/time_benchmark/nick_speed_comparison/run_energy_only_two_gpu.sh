...
(
    ...
    export SPEED_EVAL_OUTPUT="$BENCHMARK_DIR/results/nick/nick_exact_triton_all_energy_only_2warmups_speed_eval.pt"
    "$PYTHON" -u evaluation_script.py > "$BENCHMARK_DIR/logs/nick/triton_all_energy_only.log" 2>&1
) &
triton_pid=$!
...
(
    ...
    export SPEED_EVAL_OUTPUT="$BENCHMARK_DIR/results/nick/nick_exact_upstream_energy_only_2warmups_speed_eval.pt"
    "$PYTHON" -u evaluation_script.py > "$BENCHMARK_DIR/logs/nick/upstream_energy_only.log" 2>&1
) &
upstream_pid=$!

echo "Triton all-config PID: $triton_pid (GPU 0)"
echo "Upstream PID:   $upstream_pid (GPU 1)"
echo "Logs: $BENCHMARK_DIR/logs/nick/triton_all_energy_only.log"
echo "      $BENCHMARK_DIR/logs/nick/upstream_energy_only.log"
