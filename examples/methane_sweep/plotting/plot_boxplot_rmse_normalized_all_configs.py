#!/usr/bin/env python3
"""Create box plots for all configurations showing normalized RMSE/STD distribution."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-100k",
        default=Path("examples/methane_sweep/results/paper/boxplot_rmse_normalized_all_configs_100k.png"),
        type=Path,
        help="Output image path for the 100k dataset box plot.",
    )
    parser.add_argument(
        "--output-1m",
        default=Path("examples/methane_sweep/results/paper/boxplot_rmse_normalized_all_configs_1m.png"),
        type=Path,
...
def create_boxplot_for_dataset(data_dict: dict[tuple[int, int, int], list[float]], 
    ...
    # Prepare data for box plot (normalize by ENERGY_STD_KCAL_MOL)
    plot_data = []
    labels = []
    stats_info = []
    ...
    for l_max, n_max in configs:
        ...
        if key in data_dict and data_dict[key]:
            ...
            # Normalize values
            normalized_values = [v / ENERGY_STD_KCAL_MOL for v in values]
            plot_data.append(normalized_values)
            labels.append(rf"$\ell={l_max}, n={n_max}$" + f"\n(N={len(values)})")
            
            mean_val = np.mean(normalized_values)
            std_val = np.std(normalized_values, ddof=1) if len(values) > 1 else 0.0
            stats_info.append({
                'l': l_max,
                'n': n_max,
                'mean': mean_val,
                ...
                'raw_std': np.std(values, ddof=1) if len(values) > 1 else 0.0,
            })
    ...
    ax.set_ylabel("Test RMSE/STD", fontsize=14)
    ax.set_title(f"HIP-HOP-NN Normalized Test RMSE Distribution - {dataset_label} Dataset", 
    ...
    plt.close(fig)
    print(f"Saved normalized box plot to {output}")
    
    # Print summary
    print(f"\n{dataset_label} Dataset Summary (Normalized):")
    for s in stats_info:
        print(f"  l={s['l']}, n={s['n']}: N={len(s['values'])}, "
              f"RMSE/STD={s['mean']:.6f} ± {s['std']:.6f}")
        print(f"    Raw RMSE: {s['raw_mean']:.4f} ± {s['raw_std']:.4f} kcal/mol")
        if len(s['values']) <= 10:
            print(f"    Normalized values: {[f'{v:.6f}' for v in s['values']]}")
